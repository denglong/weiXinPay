...
- (void)viewDidLoad {
    [super viewDidLoad];
    UIButton *myButton = [[UIButton alloc] initWithFrame:CGRectMake(100, 200, 100, 50)];
    [myButton setTitle:@"微信支付" forState:UIControlStateNormal];
    myButton.backgroundColor = [UIColor blackColor];
    [self.view addSubview:myButton];
    [myButton addTarget:self action:@selector(sendPay) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)sendPay
{
    bool my = [WXApi registerApp:@"wxd930ea5d5a258f4f" withDescription:@"weiXinPay"];
    if (my) {
        NSLog(@"微信支付注册成功！");
    }
    //商户号
